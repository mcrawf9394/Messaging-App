   ...
   // const socket = io('https://messaging-app-api.adaptable.app')
    ...
    return <div className='w-11/12 sm:w-8/12 mx-auto bg-blue-400 h-screen'>
        ...
        <Form className='absolute bottom-0 w-11/12 sm:w-8/12 my-2 inline-flex justify-around items-center'>
            ...
            <button className='sm:h-8 my-8 transition ease-in-out delay-150 border-4 border-solid border-black hover:text-white hover:bg-black sm:w-1/12 rounded-lg' onClick={async (click) => {
                click.preventDefault()
                // socket.emit("send-message", params.chatRoomId, newMessage, localStorage.getItem('name'), res => {
                //     console.log(res)
                // })
                try {
                    const request = await fetch(Info + '/addmessage', {
                        mode: 'cors',
                        method: 'POST',
